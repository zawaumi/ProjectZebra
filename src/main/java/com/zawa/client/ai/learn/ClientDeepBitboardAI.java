...
public class ClientDeepBitboardAI extends AbstractClientAi {
    ...
    public ClientDeepBitboardAI(Integer myturn, Integer[][] othello_array) {
        super(myturn, othello_array);
        this.ai_name = "DeepBitboardAI";
    }

    public ClientDeepBitboardAI() {
    ...
    private int searchRoot(long myBoard, long oppBoard, long legalMoves, int depth, int emptyCount) {
        ...
        long[] sortedMoves = getSortedMoves(legalMoves, ttMove);
    ...
    private long[] getSortedMoves(long legalMoves, int ttMove) {
        ...
        while (temp != 0L) {
            ...
            temp &= temp - 1L;
            int score = MOVE_ORDER_WEIGHTS[m];
            if (m == ttMove) score += 10000;
    ...
    private int evaluateBoard(long myBoard, long oppBoard) {
        ...
        int myFrontier = getFrontierCount(myBoard, empty);
        int oppFrontier = getFrontierCount(oppBoard, empty);
        
        int score = (myMob - oppMob) * 20;
        score += (oppFrontier - myFrontier) * 15;
        
        long myCorners = myBoard & 0x8100000000000081L;
        long oppCorners = oppBoard & 0x8100000000000081L;
        score += Long.bitCount(myCorners) * 1000;
        score -= Long.bitCount(oppCorners) * 1000;
        
        long uncapturedCorners = empty & 0x8100000000000081L;
        if ((uncapturedCorners & 0x0000000000000001L) != 0) {
            if ((myBoard & 0x0000000000000302L) != 0) score -= 300;
            if ((oppBoard & 0x0000000000000302L) != 0) score += 300;
        }
        if ((uncapturedCorners & 0x0000000000000080L) != 0) {
            if ((myBoard & 0x000000000000C040L) != 0) score -= 300;
            if ((oppBoard & 0x000000000000C040L) != 0) score += 300;
        }
        if ((uncapturedCorners & 0x0100000000000000L) != 0) {
            if ((myBoard & 0x0203000000000000L) != 0) score -= 300;
            if ((oppBoard & 0x0203000000000000L) != 0) score += 300;
        }
        if ((uncapturedCorners & 0x8000000000000000L) != 0) {
            if ((myBoard & 0x40C0000000000000L) != 0) score -= 300;
            if ((oppBoard & 0x40C0000000000000L) != 0) score += 300;
        }
        return score;
    }

    private int getFrontierCount(long board, long empty) {
        long f = 0L;
