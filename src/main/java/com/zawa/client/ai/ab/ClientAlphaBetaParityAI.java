...
public class ClientAlphaBetaParityAI extends AbstractClientAi {

    private static final int MIDGAME_DEPTH = 6;
    private static final int ENDGAME_DEPTH = 12;
    ...
    public ClientAlphaBetaParityAI(Integer myturn, Integer[][] othello_array) {
        super(myturn, othello_array);
    }

    public ClientAlphaBetaParityAI() {
        super(0, new Integer[8][8]);
        this.ai_name = "AlphaBetaParityAI";
    }
    ...
    private int alphaBeta(int[][] board, int depth, int alpha, int beta, int currentTurn, int myTurn, int emptyCount) {
        if (depth == 0 || emptyCount == 0) {
            return evaluate(board, myTurn, emptyCount);
        }
    ...
    private int evaluate(int[][] board, int myTurn, int emptyCount) {
        if (emptyCount == 0) {
            return evaluateExact(board, myTurn);
        }
    ...
    private int calculateMobilityScore(int[][] board, int myTurn) {
        return getValidMoves(board, myTurn).size() - getValidMoves(board, -myTurn).size();
    }
